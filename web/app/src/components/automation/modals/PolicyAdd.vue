      ...
      <q-form @submit="submit">
        <q-card-section v-if="options.length > 0">
          <observer-dropdown
            ...
            :options="options"
            :label="$t('policyAdd.serverPolicy')"
            outlined
            clearable
            mapOptions
            filterable
          ...
          <observer-dropdown
            ...
            :options="options"
            :label="$t('policyAdd.workstationPolicy')"
            outlined
            clearable
            mapOptions
            filterable
          ...
          <q-checkbox
            :label="$t('policyAdd.blockInheritance')"
            v-model="blockInheritance"
          >
            <q-tooltip>{{
              $t("policyAdd.inheritanceTooltip", { type: type })
            }}</q-tooltip>
          </q-checkbox>
        </q-card-section>
        <q-card-section v-else>
          {{ $t("policyAdd.noPolicies") }}
        </q-card-section>
    ...
    submit() {
      ...
      this.$axios
        ...
        .then(() => {
          this.$q.loading.hide();
          this.onOk();
          this.notifySuccess(this.$t("policyAdd.notifyUpdated"));
        })
        .catch(() => {
          this.$q.loading.hide();
        });
