...
from observerrmm.constants import AuditActionType, PAStatus
from observerrmm.helpers import get_certs, notify_error
...
class TwilioSMSTest(APIView):
    ...
    def post(self, request):
        ...
        msg, ok = core.send_sms("Observer RMM Test SMS", test=True)
        if not ok:
            return notify_error(msg)
...
def status_v2(request):
    from agents.models import Agent
    from clients.models import Client, Site
    from observerrmm.helpers import get_nats_ports
    from observerrmm.utils import get_celery_queue_len, localhost_port_is_open
