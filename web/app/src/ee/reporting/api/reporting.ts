...
export function useReportTemplates() {
  ...
  function runReportPreview(payload: RunReportPreviewRequest) {
    isLoading.value = true;
    isError.value = false;
    // Feature 004 — revocar Object URL anterior para evitar memory leak (Q-REP-07)
    if (renderedPreview.value.startsWith("blob:"))
      URL.revokeObjectURL(renderedPreview.value);
    renderedPreview.value = "";
    axios
      .post(`${baseUrl}/templates/preview/`, payload, {
  ...
  function downloadReport(
    ...
    Dialog.create({
      ...
      if (needsPrompt.length > 0) {
        ...
        // (onDismiss disparaba runReport con deps vacías al cancelar)
        Dialog.create({
