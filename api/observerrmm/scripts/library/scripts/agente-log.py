#!/usr/bin/env python3
"""Ultimas lineas de los logs del agente ObserverRMM y del agente Mesh.

Solo LEE. Util para soporte cuando el agente responde pero se comporta mal: evita
tener que abrir una sesion remota nada mas que para mirar un log.

Uso:
    agente-log.py [lineas]

`lineas` es opcional y por defecto son 50. Se acota a 1000 para no devolver una
...
# Ubicaciones tomadas del codigo del agente (main.go:186-188, agent/agent.go:91-102).
...
def leer_cola(ruta, cantidad):
    """Ultimas `cantidad` lineas de `ruta`, leyendo solo el final del archivo."""
    tamano_bloque = 8192
...
def main():
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print("El argumento debe ser un numero de lineas. Recibido:", sys.argv[1])
            return 1
        if cantidad < 1:
            print("El numero de lineas debe ser mayor que 0.")
            return 1
    ...
    for etiqueta, ruta in LOGS:
        ...
        print("")
        print(
            "===== {} - {} (ultimas {} lineas) =====".format(etiqueta, ruta, cantidad)
        )
        try:
    ...
    if not encontrado:
        print("No se encontro ningun log del agente en las rutas conocidas:")
        for etiqueta, ruta in LOGS:
