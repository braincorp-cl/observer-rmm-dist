<template>
  <q-dialog ref="dialogRef" @hide="onDialogHide">
    <q-card class="dialog-plugin" style="min-width: 30vw">
      <q-bar>
        {{ $t("rebootLater.title", { hostname: agent.hostname }) }}
        <q-space />
      ...
      <q-card-section>
        <q-input
          type="datetime-local"
          dense
          :label="$t('rebootLater.rebootTime')"
          stack-label
          filled
          v-model="state.datetime"
          :hint="$t('rebootLater.timezoneHint')"
        />
      </q-card-section>
      <q-card-actions align="right">
        ...
        <q-btn
          ...
          flat
          push
          :label="$t('rebootLater.scheduleReboot')"
          color="primary"
  ...
  setup(props) {
    ...
    async function scheduleReboot() {
      ...
      try {
        ...
        $q.dialog({
          title: t("rebootLater.rebootPending"),
          style: "width: 40vw",
          message: t("rebootLater.scheduledMsg", {
            time: ret.time,
