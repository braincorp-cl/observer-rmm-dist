...
class TaskSerializer(serializers.ModelSerializer):
    ...
    # Q-TAR-04 / RN-026 / EC-11 — invariantes por task_type para PUT-sin-task_type.
    # Cada task_type lista los campos schedule que NO pueden ser null/empty si el
    # cliente los envía explícitos en un PUT que no especifica task_type. Sin esta
    # tabla, el bloque siguiente borraría silenciosamente el null y saltaría toda
    # la validación (return data temprano), dejando el task en estado inválido.
    SCHEDULE_INVARIANTS = {
        TaskType.RUN_ONCE: ["run_time_date"],
        TaskType.DAILY: ["run_time_date", "daily_interval"],
        TaskType.WEEKLY: ["run_time_date", "weekly_interval", "run_time_bit_weekdays"],
        TaskType.MONTHLY: [
            "run_time_date",
        ...
        TaskType.CHECK_FAILURE: ["assigned_check"],
    }

    def validate(self, data):
        # allow editing with task_type not specified
        if self.instance and "task_type" not in data:
            # Q-TAR-04: reject explicit null/empty values that would violate
            # the existing task_type's invariants before the del-and-skip below.
            instance_type = self.instance.task_type
            for field in self.SCHEDULE_INVARIANTS.get(instance_type, []):
                if field in data and not data[field]:
                    raise serializers.ValidationError(
                        f"{field} cannot be null or empty for existing "
                        f"task_type '{instance_type}'"
                    )
