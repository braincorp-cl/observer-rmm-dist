...
from observerrmm.constants import AGENT_STATUS_ONLINE, ORPHANED_WIN_TASK_LOCK
from observerrmm.helpers import rand_range, setup_nats_options
...
def remove_orphaned_win_tasks(self) -> str:
    with redis_lock(ORPHANED_WIN_TASK_LOCK, self.app.oid) as acquired:
        ...
        items: "list[AgentTup]" = []
        exclude_tasks = ("ObserverRMM_SchedReboot",)
        ...
        async def _handle_task(nc: "NATSClient", sub, data, names) -> str:
            ...
            for name in r:
                ...
                if name.startswith("ObserverRMM_") and name not in names:
                    nats_data = {
                        "func": "delschedtask",
