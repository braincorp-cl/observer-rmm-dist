...
class Agent(BaseAuditModel):
    ...
    def do_update(self, *, token: str = "", force: bool = False) -> str:
        ...
        bin = f"observeragent-v{ver}-{self.plat}-{self.goarch}.exe"
    ...
    def recover(
        ...
        if mode == "tacagent":
            if self.plat == AgentPlat.LINUX:
                ...
                bin = f"observeragent-v{settings.LATEST_AGENT_VER}-{self.plat}-{self.goarch}.exe"
                cmd = (
                    f"$url='{agent_url}';"
                    r"$destDir=Join-Path $env:ProgramData 'ObserverRMM';"
                    f"$fileName='{bin}';"
