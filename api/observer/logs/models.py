...
from observer.middleware import get_debug_info, get_username
from observer.models import PermissionQuerySet
...
class AuditLog(models.Model):
    ...
    def audit_script_run(
        ...
        AuditLog.objects.create(
            agent=agent.hostname if agent else "Observer RMM Server",
            agent_id=agent.agent_id if agent else "N/A",
            username=username,
            object_type=AuditObjType.AGENT,
            action=AuditActionType.EXEC_SCRIPT,
            message=f'{username} ran script: "{script}" on {agent.hostname if agent else "Observer RMM Server"}',
            debug_info=debug_info,
        )
    ...
    def audit_test_script_run(
        ...
        AuditLog.objects.create(
            ...
            object_type=AuditObjType.AGENT,
            action=AuditActionType.EXEC_SCRIPT,
            message=f'{username} tested a script on {agent.hostname if agent else "Observer RMM Server"}',
            debug_info=debug_info,
