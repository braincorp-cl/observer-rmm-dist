...
class TestSoftwareViews(ObserverTestCase):
    def setUp(self):
        self.authenticate()
    ...
    def test_refresh_installed_software(self, nats_cmd):
        ...
        with open(
            os.path.join(settings.BASE_DIR, "observerrmm/test_data/software1.json")
        ) as f:
            sw = json.load(f)
...
class TestSoftwarePermissions(ObserverTestCase):
    def setUp(self):
        self.setup_coresettings()
        self.setup_client()
