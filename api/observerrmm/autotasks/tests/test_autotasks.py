...
class TestAutotaskViews(ObserverTestCase):
    def setUp(self):
        self.authenticate()
    ...
    def test_update_autotask_rejects_null_schedule_fields(self):
        # Q-TAR-04 / EC-11 / RN-026: a PUT without task_type that sends a schedule
        # field as null/empty must fail validation instead of silently dropping
        # the field and leaving the task in an invalid state.
        agent = baker.make_recipe("agents.agent")
        ...
        for task_type, null_field, seed_fields in invariants:
            with self.subTest(task_type=task_type, null_field=null_field):
                task = baker.make(
                ...
                # PUT without task_type, sending the invariant field as null → 400
                resp = self.client.put(url, {null_field: None}, format="json")
                self.assertEqual(
                    resp.status_code,
                    400,
                    msg=f"PUT {null_field}=null on {task_type} must be rejected",
                )

                # control: PUT without task_type that only renames the task → 200
                # (does not violate any invariant, fields are silently preserved)
                resp = self.client.put(url, {"name": "Renamed"}, format="json")
                self.assertEqual(resp.status_code, 200)

                # reload and assert the invariant field was preserved in DB
                task.refresh_from_db()
                self.assertIsNotNone(
                    getattr(task, null_field),
                    msg=f"{null_field} must remain non-null after rename-only PUT",
                )
...
class TestAutoTaskCeleryTasks(ObserverTestCase):
    def setUp(self):
        self.authenticate()
        self.setup_coresettings()
...
class TestTaskPermissions(ObserverTestCase):
    def setUp(self):
        self.setup_coresettings()
        self.setup_client()
