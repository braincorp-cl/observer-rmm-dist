...
from observer.helpers import has_script_actions, has_webhook, setup_nats_options
    ...
    def recover(self, mode: str, mesh_uri: str, wait: bool = True) -> tuple[str, bool]:
        ...
        if mode == "tacagent":
            if self.plat == AgentPlat.LINUX:
                cmd = "systemctl restart observeragent.service"
                shell = 3
            elif self.plat == AgentPlat.DARWIN:
                cmd = "launchctl kickstart -k system/observeragent"
                shell = 3
            else:
                cmd = "net stop observeragent & taskkill /F /IM observeragent.exe & net start observeragent"
                shell = 1
