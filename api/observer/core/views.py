...
from observer.constants import AuditActionType, PAStatus
from observer.helpers import get_certs, notify_error
from observer.logger import logger
from observer.permissions import (
    _has_perm_on_agent,
    _has_perm_on_client,
    _has_perm_on_site,
)
...
def dashboard_info(request):
    ...
    from observer.utils import get_latest_trmm_ver, runcmd_placeholder_text
...
def email_test(request):
    ...
    msg, ok = core.send_mail(
        subject="Test from Observer RMM", body="This is a test message", test=True
    )
    if not ok:
...
def server_maintenance(request):
    from observer.utils import reload_nats
...
class RunURLAction(APIView):
    ...
    def patch(self, request):
        ...
        from observer.utils import RE_DB_VALUE, get_db_value
...
def status_v2(request):
    from agents.models import Agent
    from clients.models import Client, Site
    from observer.helpers import get_nats_ports
    from observer.utils import get_celery_queue_len, localhost_port_is_open
